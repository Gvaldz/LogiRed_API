...
func (r *UserRepository) CreateUser(u user.User) (user.User, error) {
	...
	query := "INSERT INTO users (name, lastname, password, email, usertype) VALUES (?, ?, ?, ?, ?)"

	result, err := r.DB.Exec(query, u.Name, u.Lastname, u.Password, u.Email, u.UserType)
	if err != nil {
...
func (r *UserRepository) GetUserByID(iduser int32) (user.User, error) {
	...
	var u user.User
	query := "SELECT iduser, name, lastname, email, usertype FROM users WHERE iduser = ?"

	err := r.DB.QueryRow(query, iduser).Scan(&u.IdUser, &u.Name, &u.Lastname, &u.Email, &u.UserType)
